...
if(isset($_REQUEST['goal'])){
  ...
  $sqlproduct="select * from tblproduct where goalid=$goal limit ".$startnum.",".$results_per_page;
  $resultproducts=mysqli_query($conn,$sqlproduct) or die(mysqli_error($conn));
  // Find out number of elements in the database
  $sqltotal="select * from tblproduct where goalid=$goal";
...
// Category Fetching 
  $sqlgoalname="select * from tblgoal where id=$goal";
  $resultgoal=mysqli_query($conn,$sqlgoalname) or die(mysqli_error($conn));
	...
	<div id="breadcrumb">
		<div class="container">
			<ul class="breadcrumb">
				...
				<li class="active"><?php echo $rowgoal['goalname']; ?></li>
			</ul>
		</div>
	</div>
  	<!-- /BREADCRUMB -->
						...
						echo'<div class="col-md-4 col-sm-6 col-xs-6">
							...
							<div class="product product-single">
								<div class="product-thumb">
									...
									<a href="product-page.php?product='.$rowproducts['id'].'"><button class="main-btn quick-view"><i class="fa fa-search-plus"></i> Quick view</button></a>
									<img src="img/products/'.$rowproducts['productprimaryimage'].'" alt="'.$rowproducts['producttitle'].'"></a>
								</div>
								<div class="product-body">
								<h3 class="product-price"><i class="fas fa-rupee-sign"></i>'.$rowproducts['productofferprice'].'<del class="product-old-price"> <i class="fas fa-rupee-sign"></i>'.$rowproducts['productprice'].'</del></h3>
									...
									<h2 class="product-name"><a href="product-page.php?product='.$rowproducts['id'].'">'.$rowproducts['producttitle'].' </a></h2>
								</div>
							</div>
							<!-- /Product Single -->
							</div>';
